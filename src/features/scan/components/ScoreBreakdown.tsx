...
interface ScoreBreakdownProps {
  product: any;
  scoreData: any;
  ageGroup: 'child' | 'teen' | 'adult' | 'elderly'; // NEW — no more silent child/adult mix-up
}

// Mirrors scoreCalculator.ts's novaScale exactly. Kept in one place per file
// would be even better (import it from scoreCalculator.ts if it's exported),
// but duplicated-and-commented is safer than silently drifting out of sync.
const novaScale: Record<number, number> = { 1: 1.00, 2: 0.90, 3: 0.70, 4: 0.50 };

export function ScoreBreakdown({ product, scoreData, ageGroup }: ScoreBreakdownProps) {
  const ageScore = scoreData.ageWise[ageGroup];
  const comp = ageScore.components;
  const isProv = comp.A === null;

  const scale = novaScale[product.nova] ?? 0.50;
  const wN = 0.35, wI = 0.20, wP = 0.15, wA = isProv ? 0 : 0.30;

  // These now match scoreCalculator.ts's actual formula:
  //   scale * (0.20*I + 0.15*P + 0.30*A) + 0.35*N_capped  [minus cliff penalty, shown separately]
  // instead of the previous `comp.X * weight` with no NOVA scale applied —
  // which only happened to be correct for NOVA 1 products.
  const valN = (comp.N * wN).toFixed(1);
  const valI = (comp.I * wI * scale).toFixed(1);
  const valP = (comp.P * wP * scale).toFixed(1);
  const valA = isProv ? '0.0' : (comp.A! * wA * scale).toFixed(1);

  const sumBeforeCliff = parseFloat(valN) + parseFloat(valI) + parseFloat(valP) + parseFloat(valA);
  const cliffPenalty = Math.max(0, Math.round((sumBeforeCliff - ageScore.score) * 10) / 10);
  ...
  const totalSugar = getNutrient(product.nutrition, 'totalSugar');
  const sodium = getNutrient(product.nutrition, 'sodium');

  // Max contribution each pillar could add is its weight * scale (not just its
  // weight), since that's the actual ceiling given this product's NOVA tier —
  // showing the un-scaled weight as "max" would make a NOVA-4 product's low
  // Ingredients/Processing/Additives contributions look like a bug rather than
  // the intended structural penalty.
  const sections = [
    {
      ...
      desc: totalSugar > 15 || sodium > 400 ? 'High in saturated fat, sugar, and sodium.' : 'Balanced nutritional profile.'
    },
    {
      id: 'Ingredients', title: 'Ingredients Pillar', weight: `${(wI * 100).toFixed(0)}%`,
      score: valI, max: (wI * scale * 100).toFixed(1), icon: Leaf, color: 'text-green-600', bg: 'bg-green-50', border: 'border-green-100',
    ...
    },
    {
      ...
      score: valP, max: (wP * scale * 100).toFixed(1), icon: Factory, color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-100',
    ...
    },
    {
      ...
      score: valA, max: (wA * scale * 100).toFixed(1), icon: FlaskConical, color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-orange-100',
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm">
      ...
      <div className="flex items-center justify-between mb-8 pb-6 border-b border-gray-50">
        <div>
          <h3 className="font-bold text-gray-900 text-xl">Detailed Score Breakdown</h3>
          <p className="text-sm text-gray-500 mt-1">How this product's NutriGuard Score is calculated ({ageGroup})</p>
        </div>

        <div className="flex items-center gap-6">
           <div className="flex flex-col items-end">
              <span className="text-[10px] text-gray-500 font-bold uppercase tracking-widest mb-1">Final Score</span>
              <span className="px-3 py-1 bg-orange-50 text-orange-700 text-xs font-bold rounded-full">Grade {ageScore.grade}</span>
           </div>
           ...
           <div className="relative w-16 h-16 rounded-full border-4 border-orange-100 flex items-center justify-center">
              ...
              <div className="flex flex-col items-center z-10">
                 <span className="text-2xl font-bold text-orange-600 leading-none">{ageScore.score}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {sections.map(sec => {
          ...
          const ratio = parseFloat(sec.max) > 0 ? (parseFloat(sec.score) / parseFloat(sec.max)) * 100 : 0;
      ...
      <div className="flex flex-col md:flex-row gap-6">
         <div className="flex-1 bg-purple-50/50 border border-purple-100 rounded-xl p-5 flex items-center gap-5">
            ...
            <div className="flex-1">
               ...
               <p className="text-xs text-purple-700/70 mt-1">Applied to Ingredients, Processing, and Additives (not Nutrition, which is separately capped at 50 instead).</p>
            </div>
            ...
            <div className="bg-purple-100/50 px-4 py-2 rounded-lg border border-purple-100 flex flex-col items-center">
               ...
               <span className="font-bold text-purple-900 text-sm">x{scale.toFixed(2)}</span>
            </div>
         </div>
         ...
         <div className="md:w-1/3 bg-gray-50/80 border border-gray-100 rounded-xl p-5 flex flex-col justify-center">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              Nutrition + (scaled) Ingredients + Processing + Additives{cliffPenalty > 0 ? ` − a ${cliffPenalty}-point Danger Cliff penalty` : ''} = {ageScore.score}.
            </p>
         </div>
      </div>
