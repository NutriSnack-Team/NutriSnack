...
export function ComparisonCard({ alt, rank = 1 }: { alt: AlternativeProduct; rank?: number }) {
  ...
  return (
    <div className="flex flex-col relative bg-white border border-gray-100 rounded-2xl shadow-sm hover:shadow-md transition-all h-full overflow-hidden">
      ...
      {/* Image Area */}
      <div className="w-full h-37.5 flex items-center justify-center p-3 mt-7 relative">
        {alt.image ? (
          <img src={getImageUrl(alt.image)} alt={alt.name} className="max-w-full max-h-full object-contain mix-blend-multiply drop-shadow-sm" />
        ) : (
      ...
      <div className="flex items-center justify-center gap-4 mt-1 px-3">
        ...
        <div className="flex flex-col items-center justify-center bg-[#f7fdf9] rounded-lg px-3 py-1.5 min-w-12.5 border border-green-50/50">
