import { X, Info, Plus, Search, CheckCircle2, MinusCircle, XCircle, BarChart2, ShieldCheck, Trophy } from 'lucide-react';
...
interface CompareTableProps {
  currentProduct: any;
  alternatives: any[];
  selectedAgeGroup: 'child' | 'teen' | 'adult' | 'elderly';
}
...
export function CompareTable({ currentProduct, alternatives, selectedAgeGroup }: CompareTableProps) {
  ...
  const dynamicCompareItems = compareItems.map(item => {
    ...
    const ageScore = scoreData.ageWise[selectedAgeGroup];
    return { ...item, score: ageScore.score, grade: ageScore.grade };
  });
  ...
  const currentAgeScore = currentScoreData.ageWise[selectedAgeGroup];
  const items = [
    { ...currentProduct, isCurrent: true, grade: currentAgeScore.grade, score: currentAgeScore.score },
    ...paddedAlternatives
  ];
  ...
  const handleAddProduct = (product: any) => {
    ...
    const ageScore = scoreData.ageWise[selectedAgeGroup];
    const productWithScore = { ...product, score: ageScore.score, grade: ageScore.grade };
              ...
              <div key={idx} className="p-6 relative border-r border-gray-100 last:border-0 flex flex-col items-center text-center">
                ...
                {item ? (
                  <>
                    <div className="items-center justify-center ">
                      ...
                      <Link to={`/product/${item.id}`} className="w-32 h-40 shrink-0 flex items-center justify-center p-2 hover:scale-105 transition-transform duration-200">
                        {item.image ? (
                          <img src={getImageUrl(item.image)} alt={item.name} className="max-w-full max-h-full object-contain mix-blend-multiply drop-shadow-md" />
                        ) : (
                          <div className="min-h-50 pt-24 text-center text-gray-400">No Image</div>
                        )}
                      </Link>
                    </div>
                  ...
                  <button 
                    ...
                    className="w-full h-full min-h-50 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center text-blue-600 hover:border-blue-400 hover:bg-blue-50 transition-colors"
                  >
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="bg-white rounded-2xl w-full max-w-md shadow-xl overflow-hidden flex flex-col max-h-[80vh]">
          ...
          <div className="flex-1 overflow-y-auto p-2">
            {productsData
              .filter((p: any) => p.name.toLowerCase().includes(searchQuery.toLowerCase()) && p.id !== currentProduct.id && !compareItems.find(c => c.id === p.id))
              .map((p: any) => (
                <button 
                  key={p.id} 
                ...
                >
                  <div className="w-12 h-12 bg-white border border-gray-100 rounded-lg flex items-center justify-center shrink-0 p-1">
                    <img src={getImageUrl(p.image)} alt={p.name} className="w-full h-full object-contain mix-blend-multiply" />
                  </div>
                  <div>
